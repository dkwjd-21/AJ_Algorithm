...
public class Main {
	public static void main(String[] args) throws IOException{
		...
		// 2*n 직사각형을 1*2, 2*1 직사각형으로 채우는 경우의 수 배열
		...
		for(int i=3; i<=n; i++) {
			...
			// n*(i-2) 직사각형을 채운 경우의 수 --> 2*1 직사각형 2개를 덧붙일 수 있음
			// 이 두 가지 경우를 합산한 값이다.
			square[i] = (square[i-1] + square[i-2]) % 10007;	// 계산을 할 때마다 미리 오버플로우 방지
		}
